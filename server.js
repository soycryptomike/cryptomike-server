...
app.get('/', (req, res) => {
  res.json({ 
    status: 'ok', 
    message: 'CryptoMike VIP Server v17 (Full Code Restore + Orders)',
...
// EJECUCIÓN INTELIGENTE BITUNIX (MANTENIENDO EL CABALLO DE TROYA)
...
// NUEVA RUTA: OBTENER ÓRDENES PENDIENTES BITUNIX
app.post('/bitunix/orders', async (req, res) => {
  ...
  try {
    ...
    try { 
      ...
      if (plan && plan.data) orders = orders.concat(plan.data); 
    } catch(e) {}
...
// NUEVA RUTA: OBTENER ÓRDENES PENDIENTES BITMART
app.post('/orders', async (req, res) => {
  ...
  try {
    let orders = [];
    try { 
      const open = await bitmartCall('GET', '/contract/private/get-open-orders', apiKey, secret, memo, null); 
      if (open && open.data) orders = orders.concat(open.data); 
    } catch(e) {}
    try { 
      const plan = await bitmartCall('GET', '/contract/private/get-plan-order', apiKey, secret, memo, null); 
      if (plan && plan.data) orders = orders.concat(plan.data); 
    } catch(e) {}
...
app.listen(PORT, () => {
  console.log(`CryptoMike VIP Server v17 running on port ${PORT}`);
